...
import { ProjectState, ProjectSortValues, ProjectPayload, ProjectMember } from "../types";
...
import { ReturnedError } from "../../utils/helperFunc";
...
export const { setProjects, addProject, removeProject, updateProjectName, addMembers, removeMember, setFetchProjectsLoading, setFetchProjectsError, setSubmitProjectLoading, setSubmitProjectError, clearSubmitProjectError,
    sortProjectsBy } = projectsSlice.actions;
...
export const createNewProject = (projectData: ProjectPayload,closeDialog?: () => void): AppThunk => {
    return async (dispatch) => {
        try {
...
export const deleteProject = (projectId: string,history: any): AppThunk => {
    return async (dispatch) => {
        try {
...
export const editProjectName = (projectId: string,name: string,closeDialog?: () => void): AppThunk => {
    return async (dispatch) => {
        try {
            dispatch(setSubmitProjectLoading());
...
export const addProjectMembers = (projectId: string,members: string[],    closeDialog?: () => void): AppThunk => {
    return async (dispatch) => {
        try {
            dispatch(setSubmitProjectLoading());
